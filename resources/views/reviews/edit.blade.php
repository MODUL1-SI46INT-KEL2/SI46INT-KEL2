...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-gray-800 rounded-lg shadow-lg p-6">
        <div class="mb-6">
            <a href="{{ route('reviews.my') }}" class="inline-flex items-center text-[#B9FF66] hover:text-white">
        ...
        <h1 class="text-2xl font-bold text-white mb-6">Edit Your Review for {{ $companyName }}</h1>
        
        @if($review->status === 'rejected' && $review->rejection_reason)
            <div class="mb-6 p-4 bg-gray-700 rounded-lg border-l-4 border-red-500">
                <h2 class="text-lg font-medium text-white mb-2">Review Rejected</h2>
                <p class="text-gray-300 text-sm mb-2">Please address the following concerns before resubmitting:</p>
                <p class="text-gray-300 italic">{{ $review->rejection_reason }}</p>
            </div>
        @endif
        ...
        <form action="{{ route('reviews.update', $review->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="job_title" id="job_title" value="{{ old('job_title', $review->job_title) }}" class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:ring-2 focus:ring-[#B9FF66] focus:border-transparent">
                @error('job_title')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="employment_period" id="employment_period" value="{{ old('employment_period', $review->employment_period) }}" class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:ring-2 focus:ring-[#B9FF66] focus:border-transparent" placeholder="e.g. June 2022 - Present">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="anonymous" id="anonymous" class="h-4 w-4 text-[#B9FF66] focus:ring-[#B9FF66] bg-gray-700 border-gray-600 rounded" {{ old('anonymous', $review->anonymous) ? 'checked' : '' }}>
