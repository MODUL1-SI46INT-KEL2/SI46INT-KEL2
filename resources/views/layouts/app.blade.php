...
<body class="font-sans antialiased bg-gray-900 text-white">
    ...
    <nav class="flex items-center justify-between px-6 py-4 border-b border-gray-700 bg-gray-800">
        ...
        <ul class="hidden md:flex items-center gap-6 text-sm font-small">
            ...
            <li>
                <a href="{{ route('forum.index') }}"
                    class="text-gray-200 hover:text-white {{ request()->is('forum*') && !request()->is('forum/moderation*') ? 'font-bold text-white' : '' }}">Open Forum</a>
            </li>
            <li>
                <a href="{{ route('faq.index') }}"
                    class="text-gray-200 hover:text-white {{ request()->is('faq*') ? 'font-bold text-white' : '' }}">FAQ</a>
            </li>
            @auth
                @if(auth()->user()->hasRole(['admin', 'moderator']))
                <li>
                    <a href="{{ route('forum.moderation.reports') }}"
                        class="text-gray-200 hover:text-white {{ request()->is('forum/moderation*') ? 'font-bold text-white' : '' }}">Forum Moderation</a>
                </li>
                @endif
                @if(auth()->user()->role === 'admin')
                <li>
                    <a href="{{ route('admin.reviews.index') }}"
                        class="text-gray-200 hover:text-white {{ request()->is('admin/reviews*') ? 'font-bold text-white' : '' }}">Review Moderation</a>
                </li>
                @endif
            @endauth
        </ul>
        ...
        <div class="flex items-center space-x-6">
            <!-- Notification Bell -->
            <div class="relative flex items-center" x-data="{ open: false, notifications: [], unreadCount: 0 }" @click.away="open = false" x-init="
            ...
            ">
                <button @click="open = !open" class="text-white hover:text-[#B9FF66] relative flex items-center justify-center h-6 w-6">
            ...
            @auth
                @if(auth()->user()->role === 'jobseeker')
                    <a href="{{ route('jobseeker.profile.edit') }}" class="text-white hover:text-[#B9FF66] flex items-center justify-center h-6 w-6">
                ...
                @elseif(auth()->user()->role === 'admin')
                    <a href="{{ route('employer.profile.edit') }}" class="text-white hover:text-[#B9FF66] flex items-center justify-center h-6 w-6">
                ...
                @else
                    <a href="#" class="text-white hover:text-[#B9FF66] flex items-center justify-center h-6 w-6">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                        </svg>
                    </a>
